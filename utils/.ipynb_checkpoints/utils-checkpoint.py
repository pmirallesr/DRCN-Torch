...
def calcConvSamePadding(input, convNet, nDim=2):
    ...
    pad_h = math.ceil(
        (
            convNet.kernel_size[0]
            - input.shape[2] * (1 - convNet.stride[0])
            - convNet.stride[0]
        )
        / 2
    )
    pad_w = math.ceil(
        (
            convNet.kernel_size[1]
            - input.shape[3] * (1 - convNet.stride[1])
            - convNet.stride[1]
        )
        / 2
    )
...
def calcPoolSamePadding(input, poolNet, nDim=2):
    ...
    pad_h = math.ceil(
        (
            (input.shape[2] - 1) * poolNet.stride[0]
            + 1
            + poolNet.dilation * (poolNet.kernel_size[0] - 1)
            - input.shape[2]
        )
        / 2
    )
    pad_w = math.ceil(
        (
            (input.shape[3] - 1) * poolNet.stride[1]
            + 1
            + poolNet.dilation * (poolNet.kernel_size[1] - 1)
            - input.shape[3]
        )
        / 2
    )
